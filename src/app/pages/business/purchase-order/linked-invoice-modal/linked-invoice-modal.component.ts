...
@Component({
  selector: "app-linked-invoice-modal",
  templateUrl: "./linked-invoice-modal.component.html",
  styleUrls: ["./linked-invoice-modal.component.scss"],
})
export class LinkedInvoiceModalComponent {
  @Input() data: any;
  ...
  _getCurrencyRate() {
    return this.data?.sDetails?.stAllCurrency?.content?.find((currency: any) => currency.currencyCode === this.data?.sDetails?.vendor?.paymentCurrency)?.rate || null;
  }

  _getRmbTotal() {
    let tAmt = 0;
    this.data?.sDetails?.invoices?.forEach((d: any) => {
      tAmt = tAmt + d.amount;
    });
    return tAmt*this._getCurrencyRate();
  }
}
