...
export class ShopifyConnectorService {
  ...
  add(data: any) {
    return this.http.post(`${this.baseApiUrl}/shopify-connector`, data);
  }

  update(id: string, data: any): Observable<any> {
    return this.http.put(
      `${this.baseApiUrl}/shopify-connector/${id}`,
      data
    );
  }

  getById(id: string): Observable<any> {
    return this.http.get(`${this.baseApiUrl}/shopify-connector/${id}`);
  }

  statusToggle(data: any) {
    return this.http.put(
      `${this.baseApiUrl}/shopify-connector/${data.id}/${data.active}`,
      ""
    );
  }
}
